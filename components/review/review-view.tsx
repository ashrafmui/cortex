  ...
  return (
    <Card className="min-w-0 flex-1">
      ...
      <CardContent className="flex flex-col gap-2">
        {total === 0 ? (
          <p className="px-4 text-sm text-muted-foreground">Nothing due. Great job staying current.</p>
        ) : (
          tiers.map((tier) => (
            <div
              key={tier.label}
            ...
            >
              ...
              <NumberTicker value={tier.count} className={`relative z-10 ${tier.color.text}`} />
            </div>
          ))
        )}
      </CardContent>
  ...
  return (
    <Card className="min-w-0 flex-1">
      <CardHeader className="flex flex-row items-center justify-between w-full pb-4">
        ...
        <CardDescription className="text-muted-foreground">Weakest first</CardDescription>
      </CardHeader>
      <CardContent className="flex flex-col gap-4">
        {queue.length === 0 ? (
          <p className="text-sm text-muted-foreground">No due reviews.</p>
        ) : (
          queue.map((topic) => (
...
export default function ReviewView({ queue, delta }: { queue: Topic[]; delta: number }) {
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex flex-col sm:flex-row gap-5 w-full mt-6">
        <ReadyToReview queue={queue} delta={delta} />
        <UpNext queue={queue} />
      </div>
    </div>
  )
